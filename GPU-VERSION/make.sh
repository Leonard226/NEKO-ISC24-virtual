...
./configure FCFLAGS="${FLAGS_GFORTRAN}" \ 
            CFLAGS="-O3" \
            CC="${FLAGS_GCC}" \
            FC=gfortran \
            LDFLAGS="-L${LAPACK_HOME}/lib -L/opt/nvidia/hpc_sdk/Linux_x86_64/24.3/comm_libs/nccl/lib" \
            --with-parmetis=${Parmetis_HOME} \
            --with-metis=${metis_HOME} \
            --with-cuda=${CUDA_HOME} \
            CUDA_ARCH=-arch=sm_80 \
            NVCC=/opt/nvidia/hpc_sdk/Linux_x86_64/24.3/compilers/bin/nvcc \
            CUDA_CFLAGS="${FLAGS_NVCC}" \
            --enable-device-mpi --with-nccl=/opt/nvidia/hpc_sdk/Linux_x86_64/24.3/comm_libs/nccl \
